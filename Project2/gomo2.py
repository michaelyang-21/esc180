...
def detect_row(board, col, y_start, x_start, length, d_y, d_x):
    ...
    while True:
        ...
        if board[y_start][x_start] == col:
            if is_sq_in_board(board, y_start + (length-1)*d_y, x_start + (length-1)*d_x) == False:
                break
...
def is_win(board):
    ...
    if detect_rows(board, "b", 4)[0] > 0 or detect_rows(board, "b", 4)[1] > 0:
        return "Black won"
    elif detect_rows(board, "w", 4)[0] > 0 or detect_rows(board, "w", 4)[1] > 0:
        return "White won"
...
def test_detect_row():
    ...
    x = 5; y = 1; d_x = 0; d_y = 1; length = 3
    put_seq_on_board(board, y, x, d_y, d_x, length, "w")
    put_seq_on_board(board, 0, 5, 1, 0, 1, "b")
    put_seq_on_board(board, 7, 5, 1, 0, 1, "w")
    put_seq_on_board(board, 6, 5, 1, 0, 1, "w")
    put_seq_on_board(board, 5, 5, 1, 0, 1, "w")
    put_seq_on_board(board, 4, 5, 1, 0, 1, "w")
    print_board(board)
    
    if detect_row(board, "w", 0,x,length,d_y,d_x) == (0, 0):
