...
def is_bounded(board, y_end, x_end, length, d_y, d_x):
    ...
    if is_sq_in_board(board, y_start, x_start) and is_sq_in_board(board, y_end, x_end):
        if board[y_end][x_end] == " " and board[y_start][x_start] == " ":
            return "OPEN"
    
    if is_sq_in_board(board, y_start, x_start) == False and is_sq_in_board(board, y_end, x_end) == False:
        return "CLOSED"
    
    if is_sq_in_board(board, y_start, x_start) and is_sq_in_board(board, y_end, x_end) == False:
        if board[y_start][x_start] != " ":
            return "CLOSED"
    
    if is_sq_in_board(board, y_start, x_start) == False and is_sq_in_board(board, y_end, x_end):
...
def detect_row(board, col, y_start, x_start, length, d_y, d_x):
    ...
    while True:
        if is_sq_in_board(board, y_start, x_start) == False:
            break
...
def detect_row_closed(board, col, y_start, x_start, length, d_y, d_x):
    ...
    while True:
        ...
        if board[y_start][x_start] == col:
            ...
            x_start -= d_x
            if i == length:
                if is_bounded(board, y_start, x_start, length, d_y, d_x) == "CLOSED":
                    closed_seq_count += 1
...
def detect_rows(board, col, length):
    ...
    # 2 corners
    # upper left
    open_seq_count += detect_row(board,col, 0, 0, length, 0, 1)[0]
    semi_open_seq_count += detect_row(board,col, 0, 0, length, 0, 1)[1]

    open_seq_count += detect_row(board,col, 0, 0, length, 1, 0)[0]
    semi_open_seq_count += detect_row(board,col, 0, 0, length, 1, 0)[1]
    ...
    open_seq_count += detect_row(board,col, 0, len(board) - 1, length, 1, 0)[0]
    semi_open_seq_count += detect_row(board,col, 0, len(board) - 1, length, 1, 0)[1]
...
def detect_closed_rows(board, col, length):
    ...
    closed_seq_count += detect_row_closed(board,col, 0, 0, length, 0, 1)
    closed_seq_count += detect_row_closed(board,col, 0, 0, length, 1, 0)
    closed_seq_count += detect_row_closed(board,col, 0, 0, length, 1, 1)

    closed_seq_count += detect_row_closed(board,col, 0, len(board) - 1, length, 1, 0)
    closed_seq_count += detect_row_closed(board,col, 0, len(board) - 1, length, 1, -1)

    for i in range(1, len(board[0]) - 1):
        closed_seq_count += detect_row_closed(board,col, 0, i, length, 1, 0)
        closed_seq_count += detect_row_closed(board,col, 0, i, length, 1, 1)
        closed_seq_count += detect_row_closed(board,col, 0, i, length, 1, -1)

    for i in range(1, len(board)):
        closed_seq_count += detect_row_closed(board,col, i, 0, length, 1, 1)
        closed_seq_count += detect_row_closed(board,col, i, 0, length, 0, 1)
        closed_seq_count += detect_row_closed(board,col, i, len(board) - 1, length, 1, -1)
...
def is_win(board):
    ...
    if detect_rows(board, "b", 5)[0] > 0 or detect_rows(board, "b", 5)[1] > 0 or detect_closed_rows(board, "b", 5) > 0:
        return "Black won"
    elif detect_rows(board, "w", 5)[0] > 0 or detect_rows(board, "w", 5)[1] > 0 or detect_closed_rows(board, "w", 5) > 0:
        return "White won"
...
def test_detect_row():
    ...
    x = 5; y = 1; d_x = 0; d_y = 1; length = 3
    put_seq_on_board(board, y, x, d_y, d_x, length, "w")
    put_seq_on_board(board, 4, 5, 0, 1, 1, "w")
    print_board(board)
    #if detect_row(board, "w", y,x,length,d_y,d_x) == (1, 0):
    if detect_row(board, "w", 0,5,3,d_y,d_x) == (0, 0):
